...
static
int bisearch(uint32_t ucs, uint32_t tbl[][2], uint32_t max){
    uint32_t min = 0;

    if (ucs < tbl[0][0] || ucs > tbl[max][1])
        return 0;
...
size_t char_width(wchar_t ucs) {
    ...
    if (ucs >= 0x1100 &&
        (ucs <= 0x115f ||                    // Hangul Jamo init. consonants
         ...
         (ucs >= 0xfe10 && ucs <= 0xfe19) || // Vertical f||ms
